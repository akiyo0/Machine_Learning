...
def get_datasets(imgs_dir, groundTruth_dir, borderMasks_dir,train_test="null"):
    ...
    np.max(groundTruth)==255 and np.max(border_masks)==255
    np.min(groundTruth)==0 and np.min(border_masks)==0
    print("ground truth and border masks are correctly withih pixel value range 0-255 (black-white)")
    #reshaping for my standard tensors
    imgs = np.transpose(imgs,(0,3,1,2))
    imgs.shape == (Nimgs,channels,height,width)
    groundTruth = np.reshape(groundTruth,(Nimgs,1,height,width))
    border_masks = np.reshape(border_masks,(Nimgs,1,height,width))
    groundTruth.shape == (Nimgs,1,height,width)
    border_masks.shape == (Nimgs,1,height,width)
...
imgs_test, groundTruth_test, border_masks_test = get_datasets(original_imgs_test, None,borderMasks_imgs_test,"test")
